...
def is_blocked(status, text, method="GET"):
    # POST返回501是后端不支持，不是WAF拦截
    if method == "POST" and status == 501:
        return False
...
def test_sql_injection():
    ...
    for case in cases:
        ...
        blocked_flag = is_blocked(status, text, method)
        if blocked_flag:
            blocked += 1
...
def test_false_positives():
    ...
    for case in cases:
        ...
        latencies.append(latency)
        blocked_flag = is_blocked(status, text, case["method"])
        if blocked_flag != case["expected"]:
            fp_count += 1
...
def main():
    ...
    if sql["penetrated_details"]:
        for p in sql["penetrated_details"]:
            print(f"  穿透: {p['name']} (status={p['status']})")
    ...
    if fp["fp_details"]:
        for p in fp["fp_details"]:
            print(f"  误报: {p['name']} (status={p['status']})")
