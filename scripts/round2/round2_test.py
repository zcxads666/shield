...
测试内容:
  1. SQL注入回归测试（5类Round 1穿透payload复测）
  2. CC攻击回归测试
  3. 误报测试（multipart boundary + Markdown #）
"""
...
def is_blocked(status, text):
    # 被拦截时通常返回 403 或 429，或包含 block/forbidden 字样
...
def test_sql_injection():
    ...
    cases.append({
        "name": "VT替代空格 - UNION",
        "payload": "1'%0bUNION%0bSELECT%0b*%0bFROM%0busers--",
        "method": "GET",
        "expected_block": True
    })
    ...
    cases.append({
        "name": "双重URL编码 - UNION",
        "payload": "1%2527%2520UNION%2520SELECT%2520*%2520FROM%2520users--",
        "method": "GET",
        "expected_block": True
    })
    ...
    # 3. Unicode编码(%u0027)
    cases.append({
        "name": "Unicode编码 - %u0027",
        "payload": "1%u0027%20OR%20%271%27=%271",
        "method": "GET",
        "expected_block": True
    })
    cases.append({
        "name": "Unicode编码 - %u0027 UNION",
...
def test_false_positives():
    ...
    cases.append({
        ...
        "path": "/search",
        "method": "GET",
        "params": {"q": "In SQL, you can use UNION to combine results from multiple queries."},
        "expected_block": False
    })
    cases.append({
        "name": "正常UPDATE语句",
...
def main():
    ...
    # 保存详细报告
    report_path = "/root/shield/scripts/round2/round2_report.json"
